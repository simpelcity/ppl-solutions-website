...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const titleText = dict.drivershub.userStats.meta.title.replace("{driver}", driverUsername);
  const descriptionText = dict.drivershub.userStats.meta.description.replace("{driver}", driverUsername);
...
export default async function UserStatisticsPage({ params }: Props) {
  const { lang } = await params
  const dict = await getDictionary(lang)
