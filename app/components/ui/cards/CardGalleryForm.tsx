...
export default function CardGalleryForm({ dict }: CardGalleryFormProps) {
  const galleryDict = dict.drivershub.dashboard.gallery;
    ...
    <>
        ...
        <Row className="row-gap-3 row-gap-md-4 d-flex justify-content-center">
          <Col xs={12} md={10} lg={6}>
            <Card className="px-0 rounded-1 border-0 shadow-sm bg-surface">
              <Card.Title className="fs-4 border-bottom border-dark-darker m-0 py-3 py-md-4">{editingId ? (galleryDict.form.titleEditItem || "Edit Gallery Item") : (galleryDict.form.titleNewItem || "Add Gallery Item")}</Card.Title>
              <Card.Body className="p-3 p-md-4 text-start">
                <Form onSubmit={editingId ? handleUpdate : handleSubmit}>
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-bold fs-5">{galleryDict.form.title || "Title"}</Form.Label>
                    <Form.Control
                      type="text"
                      placeholder={galleryDict.form.titlePlaceholder || "Image title"}
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-bold fs-5">{editingId ? galleryDict.form.imageEdit : galleryDict.form.image}</Form.Label>
                  ...
                  <BSButton variant="primary" type="submit" disabled={submitting} classes="mt-2">
                    {submitting ? (
                      ...
                      galleryDict.form.submitEdit || "Update Item"
                    ) : (
                      galleryDict.form.submit || "Add Item"
                    )}
                  </BSButton>
                  ...
                  {editingId && (
                    <Button
                      ...
                      disabled={submitting}
                    >
                      {galleryDict.form.cancel || "Cancel"}
                    </Button>
                  )}
                </Form>
          ...
          <Col xs={12} md={10} lg={6}>
            <Card className="px-0 rounded-1 border-0 shadow-sm bg-surface">
              <Card.Title className="fs-4 border-bottom border-dark-darker m-0 py-3 py-md-4">{galleryDict.card.title || "Gallery Items"}</Card.Title>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>{galleryDict.modal.title || "Confirm Action"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {confirmAction === "delete-item" && (galleryDict.modal.text || "Are you sure you want to delete this gallery item?")}
        </Modal.Body>
        <Modal.Footer>
          <BSButton variant="secondary" size="md" border="secondary" onClick={() => setShowModal(false)}>
            {galleryDict.modal.cancel || "Cancel"}
          </BSButton>
          <Button variant="danger" className=" text-uppercase fw-bold rounded-1 text-theme" style={{ padding: "0.5rem 1rem", fontSize: "1rem" }} onClick={handleConfirm}>
            {galleryDict.modal.confirm || "Confirm"}
          </Button>
        </Modal.Footer>
      </Modal>
    </>
