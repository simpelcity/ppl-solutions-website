...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!display_name || typeof display_name !== "string") return errorHandler({ error: dict.errors.team.NAME_REQUIRED }, request, lang, 400);

    const { data: users, error } = await supabaseAdmin.auth.admin.listUsers();

    if (error) return errorHandler({ error: dict.errors.members.FAILED_TO_FETCH_MEMBERS }, request, lang, 500);

    const existingUser = users.users.find((user) => user.user_metadata?.display_name === display_name.trim());

    if (existingUser) return NextResponse.json({ available: false, message: dict.errors.team.NAME_REQUIRED });
